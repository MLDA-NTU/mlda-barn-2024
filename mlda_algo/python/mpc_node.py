...
class ROSNode:
    ...
    def callback_global_plan(self, data):
        ...
        if self.x_ref == []:
            ...
            for i in range(self.N):
                dist = math.sqrt(
                    (self.x_ref[i] - x_ref[i]) ** 2 + (self.y_ref[i] - y_ref[i]) ** 2
                )
                if prev_dist < dist:
                    break
                else:
            ...
            for i in range(self.N - align_index):
                dist = math.sqrt(
                    (self.x_ref[i + align_index] - x_ref[i]) ** 2
                    + (self.y_ref[i + align_index] - y_ref[i]) ** 2
                )
                deviation += dist
            mean_deviation = deviation / (self.N - align_index)
            print("Mean deviation: ", mean_deviation)
            if (
                mean_deviation > self.deviation_threshold
                or align_index > self.align_index_threshold
            ):
                self.x_ref = x_ref
                self.y_ref = y_ref
                self.theta_ref = theta_ref
...
def start_traj():
    start_traj_publisher = rospy.Publisher(
        "/start_traj", Path, queue_size=1, latch=True
    )
    traj = rospy.wait_for_message(
        "/move_base/TrajectoryPlannerROS/global_plan", Path, timeout=1
    )  # GEt msg 1 time
    # mpc_traj_msg = Path()
    # mpc_traj_msg.header.stamp = rospy.Time.now()
    # mpc_traj_msg.header.frame_id = "odom"
